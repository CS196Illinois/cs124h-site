...
export async function POST(request, { params }) {
  ...
  if (isSandboxRole(userRole) && (await getSandboxMode(netID)) !== "off") {
    ...
    const fullRow = {
      id: rowPk, sprint_id: id, group_number: groupNumber,
      is_open: true, opened_at: now, opened_by: netID, closed_at: null, closed_by: null,
    };
  ...
  const { data, error: dbError } = await supabaseServer
    ...
    .upsert(
      { sprint_id: id, group_number: groupNumber, is_open: true, opened_at: now, opened_by: netID, closed_at: null, closed_by: null },
