...
export async function PATCH(request, { params }) {
  ...
  if (!EDIT_ROLES.includes(session?.user?.role)) return NextResponse.json({ error: "Only course leads and head PMs can edit the shared question bank." }, { status: 403 });
...
export async function DELETE(request, { params }) {
  const session = await getServerSession(authOptions);
  if (!EDIT_ROLES.includes(session?.user?.role)) return NextResponse.json({ error: "Only course leads and head PMs can remove shared questions." }, { status: 403 });
