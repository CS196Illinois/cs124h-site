...
import checkInStyles from "./CheckInPanel.module.css";

/**
 * Shared check-in UI, used by every role (any signed-in role can check in
 * to any open event - staff attend events too). Mounted at the one
 * role-agnostic route /user/checkin so a single URL works no matter who
 * scans it; also embedded read-only-ish under /user/student/attendance for
 * students' own "Past Attendance" history view.
 *
 * `?event=<id>` (e.g. from the QR code behind an event's magnifying-glass
 * view) deep-links straight to that one event's code entry, auto-focused -
 * see EventsPanel's enlarged code view for where that link is generated.
 */
function CheckInPanelInner() {
  const { status } = useSession();
  const searchParams = useSearchParams();
  const focusedEventId = searchParams?.get("event") || null;
  ...
  const [showAll, setShowAll] = useState(!focusedEventId);
  ...
  // Deep-linked from a QR scan - jump straight to typing the code.
  useEffect(() => {
    if (focusedEventId && !showAll) focusedInputRef.current?.focus();
  }, [focusedEventId, showAll, events]);
  ...
  const allOpenEvents = events.filter((e) => e.check_in_open);
  const closedEvents = events.filter((e) => !e.check_in_open && checkedIn.has(e.id));

  // A ?event=<id> link (QR scan) shows just that one event until the
  // person chooses to see the full list - cleaner on a phone, and makes it
  // obvious which event they're about to check in to.
  const focusedEvent = focusedEventId ? allOpenEvents.find((e) => e.id === focusedEventId) : null;
  const openEvents = focusedEvent && !showAll ? [focusedEvent] : allOpenEvents;
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.panel}>
        {focusedEvent && !showAll && (
          <button className={checkInStyles.backLink} onClick={() => setShowAll(true)}>
            ← See all open events
          </button>
        )}
        {focusedEventId && !focusedEvent && (
          <p className={checkInStyles.eventDetail} style={{ marginBottom: "0.85rem" }}>
            That event isn't open for check-in right now.
          </p>
        )}
          ...
          <div className={checkInStyles.section}>
            ...
            <div className={checkInStyles.eventList}>
              {openEvents.map((event) => {
                ...
                const isFocused = focusedEvent && event.id === focusedEvent.id && !showAll;
                return (
                  <div
                    key={event.id}
                    className={`${checkInStyles.eventCard} ${isFocused ? checkInStyles.focusedCard : ""}`}
                  >
        ...
        {closedEvents.length > 0 && (!focusedEvent || showAll) && (
          <div className={checkInStyles.section}>
        ...
        {openEvents.length === 0 && closedEvents.length === 0 && !focusedEventId && (
