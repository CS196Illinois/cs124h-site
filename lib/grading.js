...
export function assignmentLabel(assignment) {
  ...
  // A due date is a calendar day stored at UTC midnight - format it in UTC so
  // it reads the same everywhere; created_at is a real instant, shown local.
  const dateStr = new Date(dateSrc).toLocaleDateString([], {
    month: "short", day: "numeric", ...(assignment.due_date ? { timeZone: "UTC" } : {}),
  });
